...
const Reports = () => {
    ...
    const IncomesOfSelectedCategory = useSelector(state => state.finance.transactionsByPeriod?.incomes?.incomesData?.[selectedIncomes] ?? {})
    const productsOfSelectedCategory = useSelector(state => state.finance.transactionsByPeriod?.expenses?.expensesData?.[selectedCategory] ?? {});
    ...
    return (
        <div className={s.container}>
            ...
            {isIncome && (
                <ReportsIncome handleChangeRender={handleChangeRender} chooseProductChart={chooseIncomeChart} />
            )}
            <div>
                {isExpense && Boolean(width < 768) && <MobileChart objectForChart={productsOfSelectedCategory} />}
                { isExpense && Boolean(width >= 768 && width <= 1279) && <TabletChart  objectForChart={productsOfSelectedCategory}/>}
                {isExpense && Boolean(width >= 1280) && <DesktopChart objectForChart={productsOfSelectedCategory} />}
                {isIncome && Boolean(width < 768) && <MobileChart objectForChart={IncomesOfSelectedCategory} />}
                { isIncome && Boolean(width >= 768 && width <= 1279) && <TabletChart  objectForChart={IncomesOfSelectedCategory}/>}
                {isIncome && Boolean(width >= 1280) && <DesktopChart objectForChart={IncomesOfSelectedCategory} />}
            </div>
        </div>
    );
};
